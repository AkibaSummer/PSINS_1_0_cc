...
CMat operator~(const CMat &m0) {
    ...
    for (int i = 0; i < m0.row; i++) { for (int j = i; j < m0.rc; j += m0.row) mtmp.dd[j] = *pm++; }
    return mtmp;
}
...
CKalman::CKalman(int nq0, int nr0) {
    ...
    rts = CVect(nr, 1.0);
    Rmax = CVect(nr, INF);
    Rmin = Rb = CVect(nr, 0.0);
    Rbeta = CVect(nr, 1.0);
    FBTau = FBMax = CVect(nq, INF);
...
/*
该函数的功能是反馈修正，之后系统会根据FBXk进行修正
本函数是根据当前KF计算之后的Xk值，进行FBXk的计算
*/
void CKalman::Feedback(double fbts) {
    /*  以下均为15 x 1的矩阵，换成double指针的意义在于，每次处理15个元素中一个，用指针便于迭代
        FBTau表示步长（意义不明）
        FBTotal表示迭代到目前为止每一次计算所得的FBXk之和（意义不明）
        FBMax表示FBTotal的上限（意义不明）
    */
    double *pTau = FBTau.dd, *pTotal = FBTotal.dd, *pMax = FBMax.dd, *pXk = FBXk.dd, *p = Xk.dd;
    // nq等于KF中X矩阵的维度，本项目中为15
    // for循环共计算15次，每次反馈X中的一次
    for (int i = 0; i < nq; i++, pTau++, pTotal++, pMax++, pXk++, p++) {
        // INF表示一个最大值，这里第一个if的意思是判断FBTau中的元素是否合法，不合法就直接跳过，将pXk置为0
        if (*pTau < INF / 2) {
            /*
                afa表示时间间隔除以步长？
                Tau指希腊字母中的τ，多数情况下表示时间间隔，所以猜测pTau代表步长的时间间隔？
                pTau没有被当做左值使用过，也就是说多数情况下这个量没有被更改，暂时当做时间间隔来考虑
                这一部分可以看一下pTau，也即FBTau里面的值，[1,1,10,1,1,1,1,1,1,10,10,10,10,10,10]
                因为fbts = 0.01，所以计算后的afa通常为0.01或0.001
            */
            double afa = fbts < *pTau ? fbts / (*pTau) : 1.0;
            // FBXk = Xk * afa
            *pXk = *p * afa;
            /*
                如果上一时刻的pTotal加上这一时刻的pXk，也就是矩阵中的上一时刻的FBTotal加上这一时刻计算后的FBXk，总值超过了设置的最大最小阈值
                则将pXk置换成上一时刻的Total值和Max值的差
            */
            if (*pTotal + *pXk > *pMax)
                *pXk = *pMax - *pTotal;
            else if (*pTotal + *pXk < -*pMax)
                *pXk = -*pMax - *pTotal;
            // p即为Xk，更新Xk，Xk减去计算后的FBXk
            *p -= *pXk;
            // Total值加上这一时刻计算后的FBXk
            *pTotal += *pXk;
        } else {
            *pXk = 0.0;
        }
...
void CQEAHRS::Update(const CVect3 &gyro, const CVect3 &acc, const CVect3 &mag, double ts) {
    ...
    // Ft
    0, Ft.dd[1] = -wx / 2, Ft.dd[2] = -wy / 2, Ft.dd[3] = -wz / 2, Ft.dd[4] = q1 / 2, Ft.dd[5] = q2 / 2, Ft.dd[6] =
            q3 / 2;
    Ft.dd[7] = wx / 2, 0, Ft.dd[9] = wz / 2, Ft.dd[10] = -wy / 2, Ft.dd[11] = -q0 / 2, Ft.dd[12] = q3 / 2, Ft.dd[13] =
            -q2 / 2;
    Ft.dd[14] = wy / 2, Ft.dd[15] = -wz / 2, 0, Ft.dd[17] = wx / 2, Ft.dd[18] = -q3 / 2, Ft.dd[18] =
            -q0 / 2, Ft.dd[20] = q1 / 2;
    Ft.dd[21] = wz / 2, Ft.dd[22] = wy / 2, Ft.dd[23] = -wx / 2, 0, Ft.dd[25] = q2 / 2, Ft.dd[26] = -q1 / 2, Ft.dd[27] =
            -q0 / 2;
    // Hk
...
CFileRdWt::CFileRdWt(const char *fname0, int columns) {
    ...
    if (columns == 0) { if (dirOut[0] != 0 && findc == 0) { strcat(fname, dirOut); }}
    else {
